...
defineCard({
  ...
  delay: 60,                // 仅 deferred 需要，且不得低于 50（半次标准行动）
  text: '60 AV 后，造成 24 点伤害',   // 数值必须与 effects 完全对应
  effects: [{ t: 'damage', amount: 24 }],
  flavor: '一句风味文本'
})
```

**上面这个 24 不是随口写的。** 挂刻牌要兑现延迟的报酬：付费挂刻牌的伤害/时能必须**严格高于**付费即时刻印的上限（当前是 7）。示例里 `24 / 2 = 12`，安全越过；而如果按直觉写成 `amount: 10`，`10 / 2 = 5` 会被测试直接拒绝——这是最容易被踩的一处，因为那张牌单看数值并不离谱。

数值预算与三条来自实测的教训见 [卡牌设计指南](docs/卡牌设计指南.md)。
...
胜率是唯一的判据，手感不是。汇总数字只告诉你「有没有问题」，看不出「问题出在哪一段」——加伤害与加血量的效果可能完全不同：实测中首领加伤害让胜率从 80% 一步跌到 0%，改成加血量才拿到中间档；而后来修好一个空转的招式之后它的胜率又从 18% 掉到 0%，那次是**下调伤害、保留节奏**才收回来的。两次的方向正好相反，靠猜是猜不出来的。
